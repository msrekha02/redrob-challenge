...
def _compute_m_avail(snap: dict, jd_profile: JDProfile) -> tuple[float, list[str]]:
    ...
    # P-1, P-2, P-3, P-4 are UNIVERSAL signals — they apply at full
    # strength for every JD without JD-specific scaling. A candidate
    # inactive for 6 months with a 5% recruiter response rate is
    # unavailable regardless of whether the JD is for an ML Engineer
    # or a Sales Director. avail_importance was previously scaling this
    # down (e.g. 0.85 for ML JDs) — removed because that made platform
    # availability a JD-dependent signal, which it is not.
    m_avail = max(0.05, min(1.0, m_avail_raw))
